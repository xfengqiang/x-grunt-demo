...
module.exports = {
  ...
  xjs: function (content, block, blockLine, blockContent) {
//        console.log('dest:'+block.asset+'?ver='+ d.getTime())
      var url = block.asset+'?ver='+ new Date().getTime();
      return content.replace(blockLine, block.indent + '<script src="' + url + '"><\/script>');
  },
  xcss: function (content, block, blockLine, blockContent) {
      var url = block.asset+'?ver='+ new Date().getTime();
      return content.replace(blockLine, block.indent + '<link rel="stylesheet" href="' + url + '">');
  }
};
